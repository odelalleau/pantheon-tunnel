...
void check_interface_for_binding( const std::string &prog_name, const std::string &if_name ) {
    ...
    if ( access( interface_rpf_path.c_str(), R_OK ) == -1 ) {
        throw runtime_error( prog_name + ": can't read " + interface_rpf_path + ", interface \"" + if_name + "\" probably doesn't exist" );
    }
    ...
    if ( rpf_interface.read() != "0\n" or rpf_all.read() != "0\n" ) {
        throw runtime_error( prog_name + ": Can only run using --interface if both \"cat " + all_rpf_path
